...
def getCursor():
    ...
    dbconn = connection.cursor(dictionary=True)  # This returns a list of dictionaries, which is much more readable because
                                                 # it means you can refer to columns by name rather than number (index)
    return dbconn
...
def people():
    ...
    cursor = getCursor()        # Create the cursor, using the getCursor function, which we will provide for you.
    qstr = "SELECT * FROM people;"  # The query to send to MySQL Server, stored in qstr.
    cursor.execute(qstr)        # Execute the SQL query in qstr
    people = cursor.fetchall()        # Returns all rows from the query - as a list of tuples (one tuple per row).
...
def person():
    ...
    cursor.execute(qstr,qargs)      # Note the second qargs argument, which provides the data to match the %s markers in the qstr 
    person = cursor.fetchone()      # Returns only one row from the query - as a tuple.
...
def person_update():
    ...
    cursor.execute(qstr,qargs)          # The query executes here, but UPDATE queries make the changes in the database, but don't send any
...
def person_insert():
    ...
    cursor.execute(qstr,qargs)      # Executes the query and adds the row, but no data is returned to Python
    new_id = cursor.lastrowid       # A trick to get the new id created by the database for the new row
